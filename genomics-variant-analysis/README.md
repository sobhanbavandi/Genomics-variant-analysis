...
**Affiliations:** Genomics Research Laboratory; Helix Institute
...
├── results/
│   ├── candidates_homozygous_rare_coding.csv   <- 67 candidate variants
...
variant rows in one pass (`scripts/filter_variants.py`):
...
| 3 | Exonic or canonical splice-site | A variant has to be able to change the protein | 22,460 |
| 4 | Not synonymous | Synonymous SNVs don't change the amino acid sequence | 10,716 |
| 5 | Rare/absent in 1000G, ExAC, ESP6500, Kaviar, HRC (≤1%) | A severe rare disorder should not be common in the population | 677 |
| 6 | **Homozygous-ALT** in the proband | Consanguinity + a single affected child with no other family history is the classic signature of autosomal recessive disease via a shared ancestral haplotype | **67** |

Step 6 is the case-specific step, explained in more depth (with its main
blind spot) under **Limitations** below.

The 67 survivors were then split into three transparent, *computational*
...
- **`SEPSECS`** — the single best phenotypic match (Pontocerebellar
...
- **`EXOSC5`** — weaker literature support, plausible but lower confidence.
